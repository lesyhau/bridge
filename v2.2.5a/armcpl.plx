...
if (-f "$inDir/build.cfg")
{
    ...
    while (<CFG>)
    {
        ...
        # Check if the current line has been commented out
        if ($_ =~ /^#/) { goto SKIP_LINE; }
        ...
        elsif ($words[0] eq "-D")
        {
            ...
            my $macro = "";
            for (my $i = 1; $i < (scalar @words); $i++) { $macro = join " ", $macro, $words[$i]; }
            push @macros, $macro;
        }
...
if (@ARGV)
{
    ...
    for (my $i = 0; $i < $args; $i++)
    {
        ...
        if (($ARGV[$i] eq "-pd") or ($ARGV[$i] eq "--PD"))
        {
            if (! $ARGV[$i+1])
            {
                print "[Error] No symbol specified. Please specify a valid symbol after \"-pd\"\n";
                exit;
            }
        ...
        if ($ARGV[$i] eq "-D")
        {
            if (! $ARGV[$i+1])
            {
                print "[Error] No macro specified. Please specify a valid macro after \"-D\"\n";
                exit;
            }

            my $macro = $ARGV[$i+1]; chomp $macro;
            push @macros, $macro;
        }
...
elsif   ($cOptimize eq "z")     { $cOptimize = "-Oz" }
else
{
...
# and the user doesn't have a retarget file
if (@cFiles)
{
    my $retargetFound = 0;
    ...
    if ($retargetFound == 0) { push @cFiles, "$toolbase/retarget.c"; }
}
...
my $filesList = "$inDir/files.list";
open FILES_LIST, ">$filesList";
...
foreach my $file (@asmFiles)
{
    ...
    print FILES_LIST "[Info] Assembly source file: $file\n";
}
...
foreach my $file (@asmGNUFiles)
{
    ...
    print FILES_LIST "[Info] GNU assembly source file: $file\n";
}
...
foreach my $file (@cFiles)
{
    ...
    print FILES_LIST "[Info] C source file: $file\n";
}
...
foreach my $macro (@macros) { print MAKE "asmGNUopts += -D$macro\n"; }
print MAKE <<EOF;
...
foreach my $macro (@macros) { print MAKE "copts += -D$macro\n"; }
print MAKE <<EOF;
