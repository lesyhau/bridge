...
static uint32_t receivedData[2][DATA_PACKAGE_LENGTH];

uint32_t master_rx_dma_continuous(void)
{
    /* Reset SDMAC */
    CPG_SetBit(SRCR7, 9);
    ...
    SDMAC1->DMADESMEM[0].SAR = &I2C0->ICRXD;
    SDMAC1->DMADESMEM[0].DAR = receivedData[0];
    SDMAC1->DMADESMEM[0].TCR = DATA_PACKAGE_LENGTH;
    SDMAC1->DMADESMEM[0].DM = 1;    // Destination address is increasement
    SDMAC1->DMADESMEM[0].SM = 0;    // Source address is fixed
    SDMAC1->DMADESMEM[0].RS = 8;    // DMA trigger source is peripheral
    SDMAC1->DMADESMEM[0].TS = 2;    // Transfer size is 4 bytes
    SDMAC1->DMADESMEM[0].DRS = DMARS_I2C0_MRX;
    SDMAC1->DMADESMEM[1].SAR = &I2C0->ICRXD;
    SDMAC1->DMADESMEM[1].DAR = receivedData[1];
    SDMAC1->DMADESMEM[1].TCR = DATA_PACKAGE_LENGTH;
    SDMAC1->DMADESMEM[1].DM = 1;    // Destination address is increasement
    SDMAC1->DMADESMEM[1].SM = 0;    // Source address is fixed
    SDMAC1->DMADESMEM[1].RS = 8;    // DMA trigger source is peripheral
    SDMAC1->DMADESMEM[1].TS = 2;    // Transfer size is 4 bytes
    SDMAC1->DMADESMEM[1].DRS = DMARS_I2C0_MRX;
    ...
    I2C_masterSetDMAContinuousReceiveBlockCount(I2C0, DATA_PACKAGE_LENGTH);
    I2C_masterEnableDMAContinuousReceive(I2C0);
    I2C_masterEnableDMAReceive(I2C0);
    I2C_masterEnableInterrupt(I2C0, I2C_INT_MAT | I2C_INT_MDR);
    I2C_masterClearInterruptStatus(I2C0, I2C_INT_ALL);
    I2C_masterEnable(I2C0);

    /* Configure a DMA channel for I2C1 */
    SDMAC1CH1->SAR = &sendData[0][1];
    SDMAC1CH1->DAR = &I2C1->ICRXD;
    ...
    I2C_slaveSetData(I2C1, sendData[0][0]);
    ...
    /* Configure I2C1 in slave TX mode with DMA transfer enabled */
    //I2C_slaveInit(I2C1, &slaveConfig);
    I2C_slaveSetData(I2C1, sendData[1][0]);
    //I2C_slaveEnableInterrupt(I2C1, I2C_INT_SDE);
    //I2C_slaveClearInterruptStatus(I2C1, I2C_INT_ALL);
    //I2C_slaveEnable(I2C1);
