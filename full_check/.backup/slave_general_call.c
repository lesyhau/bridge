...
static volatile uint32_t receivedData[DATA_PACKAGE_LENGTH + 1];
...
uint32_t slave_general_call(void)
{
    ...
    /* Check if GCAR is set */
    if (I2C_slaveIsGeneralCall(I2C0)) { receivedData[j++] = TEST_PASS; }
    else { receivedData[j++] = TEST_FAIL; }
    ...
    /* Judge the result */
    uint32_t testResult = receivedData[0];
