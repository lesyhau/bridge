...
void GIC_enable(void)
{
    ...
    __RW uint32_t *gicrBase = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x100000);
    ...
    *gicrWAKER = 0;
    ...
    /* Enable GICD */
    __RW uint32_t *gicdCTRL = (__RW uint32_t *)(GIC_CR52_BASE_ADDR);
    *gicdCTRL = 0x3;
...
void GIC_configInterrupt(uint32_t id)
{
   ...
   //regNo = id/32;
   //bitPos = id%32;
   //__RW uint32_t *gicdIGROUPR = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x80 + regNo*4);
   //*gicdIGROUPR |= 1 << bitPos;

   ///* Set interrupt priority */
   //regNo = id/4;
   //bitPos = (id%4)*8;
   //__RW uint32_t *gicdIPRIORITYR = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x400 + regNo*4);
   //*gicdIPRIORITYR &= ~(1 << bitPos);

   ///* Set level sensitive */
   //regNo = id/16;
   //bitPos = (id%16)*2;
   //__RW uint32_t *gicdICFGR = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0xC00 + regNo*4);
   //*gicdICFGR |= 1 << bitPos;
}
...
void GIC_enableInterrupt(uint32_t id)
{
   uint32_t regNo, bitPos;
    ...
    regNo = id;
    __RW uint64_t *gicdIROUTER = (__RW uint64_t *)(GIC_CR52_BASE_ADDR + 0x6000 + 8*regNo);
    *gicdIROUTER = 0; 

    /* group status bit = 1 */
   regNo = id/32;
   bitPos = id%32;
   __RW uint32_t *gicdIGROUPR = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x0080 + regNo*4);
   *gicdIGROUPR |= 1 << bitPos;

    /* group modifier bit = 0 */
    regNo = id/32;
    bitPos = id%32;
    __RW uint32_t *gicdIGRPMODR = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x0d00 + 4*regNo);
    *gicdIGRPMODR &= ~(1 << bitPos);

    /* priority = 0 */
   regNo = id/4;
   bitPos = (id%4)*8;
   __RW uint32_t *gicdIPRIORITYR = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x0400 + regNo*4);
   *gicdIPRIORITYR &= ~(1 << bitPos);

    /* edge-triggered */
   regNo = id/16;
   bitPos = (id%16)*2;
   __RW uint32_t *gicdICFGR = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x0c00 + regNo*4);
   *gicdICFGR |= 1 << bitPos;

   regNo = id/32;
   bitPos = id%32;
   __RW uint32_t *gicdISENABLER = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x0100 + regNo*4);
...
void GIC_disableInterrupt(uint32_t id)
{
   ...
   regNo = id/32;
   bitPos = id%32;
   __RW uint32_t *gicdICENABLER = (__RW uint32_t *)(GIC_CR52_BASE_ADDR + 0x180 + regNo*4);
