...
void i2c0InterruptHandler(void)
{
	...
	{
		if (sendDataIndex < DATA_PACKAGE_LENGTH) { I2C_masterSendMultipleByteNext(I2C0, sendData[sendDataIndex++]); }
...
void i2c1InterruptHandler(void)
{
    ...
    if (status & I2C_INT_SDR) { receivedData[receivedDataIndex++] = I2C_slaveReceiveMultipleByteNext(I2C1); }
