...
sub translateLinkerOptions
{
	my $alias		= shift(@_);
	my $rawOption	= shift(@_);

	if		($alias eq "scatter")	{ return "-T $rawOption"		; }
	elsif	($alias eq "remove")	{ return "-Wl,--gc-sections"	; }
}

sub get_default_remove
{
	return 0;
}
