...
static volatile bool isAutoRXStarted = false;
static volatile bool isAutoRXComplete = false;
static volatile bool isAutoTXStarted = false;
static volatile bool isAutoTXComplete = false;
...
uint32_t master_rx_dma_continuous(void)
{
    ...
    {
        SDMAC1->DMADESMEM[i].SAR = &I2C0->ICRXD;
        SDMAC1->DMADESMEM[i].DAR = receivedData[i];
    ...
    SDMAC1CH1->SAR = &sendData[currentDataPackageIndex][1];
...
void i2c0InterruptHandler(void)
{
    ...
    else if (status & I2C_INT_MDR)
    {
        ...
        {
            SDMAC1->DMAOR = 1;
            isAutoRXStarted = true;
        }
        else { I2C_masterSendMultipleByteStop(I2C0); }
    }

    if (status & I2C_INT_MST) { isTransferComplete = isAutoTXComplete & isAutoRXComplete; }
...
void i2c1InterruptHandler(void)
{
    ...
    if (status & I2C_INT_SDE)
    {
        if (!isAutoTXStarted)
        {
            ...
            isAutoTXStarted = true;
        }
    }

    if (status & I2C_INT_SSR)
    {
        if (!isAutoTXComplete)
        {
            I2C_slaveSetData(I2C1, sendData[currentDataPackageIndex][0]);
