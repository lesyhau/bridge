...
uint32_t slave_rx_fbsc_check(void)
{
    GIC_enable();
...
void i2c1InterruptHandler(void)
{
    ...
    {
        if (sendDataIndex < DATA_PACKAGE_LENGTH) { I2C_masterSendMultipleByteNext(I2C1, sendData[sendDataIndex++]); }
        else if (sendDataIndex == DATA_PACKAGE_LENGTH) { I2C_masterSendMultipleByteStop(I2C1); }
    }
...
void i2c0InterruptHandler(void)
{
    ...
    if (status & I2C_INT_SDR) { receivedData[receivedDataIndex++] = I2C_slaveReceiveMultipleByteNext(I2C0); }
