...
#define DATA_PACKAGE_LENGTH 1
static const uint32_t sendData[DATA_PACKAGE_LENGTH] = { 'H' };
...
uint32_t master_tx_fbsc_check(void)
{
    GIC_enable();
...
void i2c1InterruptHandler(void)
{
    ...
    {
        if (receivedDataIndex < (DATA_PACKAGE_LENGTH - 2)) { receivedData[receivedDataIndex++] = I2C_slaveReceiveMultipleByteNext(I2C1); }
        else if (receivedDataIndex == (DATA_PACKAGE_LENGTH - 2)) { receivedData[receivedDataIndex++] = I2C_slaveReceiveMultipleByteStop(I2C1); }
        else if (receivedDataIndex == (DATA_PACKAGE_LENGTH - 1)) { receivedData[receivedDataIndex++] = I2C_slaveReceiveMultipleByteFinish(I2C1); }
    }
