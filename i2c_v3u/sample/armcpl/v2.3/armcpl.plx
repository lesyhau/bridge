...
use lib "/home/u/haule2/tools/armcpl/v2.3";
use settings;
...
;******************************************************************************
;	This file has been generated by Pattern Builder $toolver
...
switch ($cpu)
{
    case "ca53" { $asmCPU = "8-A.32" }
    case "ca57" { $asmCPU = "8-A.32" }
    case "ca76" { $asmCPU = "8.2-A.64" }
    case "enyo_32" { $asmCPU = "8.2-A.32" }
    case "enyo_64" { $asmCPU = "8.2-A.64" }
    case "cr7"  { $asmCPU = "Cortex-R7" }
    case "cr52" { $asmCPU = "Cortex-R52" }
    else
    {
        print "[Error] Not supported CPU $cpu\n";
        exit;
    }
}
...
switch ($cpu)
{
    case "ca53" { $asmGNUCPU = "cortex-a53" }
    case "ca57" { $asmGNUCPU = "cortex-a57" }
    case "ca76" { $asmGNUCPU = "cortex-a75" }
    case "enyo_32" { $asmGNUCPU = "cortex-a75" }
    case "enyo_64" { $asmGNUCPU = "cortex-a75" }
    case "cr7"  { $asmGNUCPU = "cortex-r7" }
    case "cr52" { $asmGNUCPU = "cortex-r52" }
    else {}
}
...
switch ($cpu)
{
    case "ca53"     { $cCPU = "cortex-a53" }
    case "ca57"     { $cCPU = "cortex-a57" }
    case "ca76"     { $cCPU = "cortex-a75" }
    case "enyo_32"     { $cCPU = "cortex-a75" }
    case "enyo_64"     { $cCPU = "cortex-a75" }
    case "cr7"      { $cCPU = "cortex-r7" }
    case "cr52"     { $cCPU = "cortex-r52" }
    else {}
}
...
switch ($cpu)
{
    case "ca53" { $cTarget = "arm-arm-non-eabi" }
    case "ca57" { $cTarget = "arm-arm-non-eabi" }
    case "ca76" { $cTarget = "aarch64-arm-non-eabi" }
    case "enyo_32" { $cTarget = "arm-arm-non-eabi" }
    case "enyo_64" { $cTarget = "aarch64-arm-non-eabi" }
    case "cr7"  { $cTarget = "arm-arm-non-eabi" }
    case "cr52" { $cTarget = "arm-arm-non-eabi" }
    else {}
}
...
if (@cFiles)
{
    push @cFiles, "$toolbase/retarget.c";
}
...
## ASM source files
#foreach my $file (@asmFiles) { print MAKE "asmFiles += $file\n"; }
#print MAKE <<EOF;
#
#EOF
#
## C source files
#foreach my $file (@cFiles) { print MAKE "cFiles += $file\n"; }
#print MAKE <<EOF;
#
#EOF
...
\$(debug)/$fileBase.o: \\
  $asmFiles[$i] \\
  $cfg
	\@echo Compiling $asmFiles[$i]...
	\$(asmc) \$(asmopts) --list=$asmFilesBase[$i].list  $asmFiles[$i]
	\@echo Collecting object files $asmFilesBase[$i++].o...
	mv -f *.o \$(debug)
	mv -f *.list \$(debug)
...
\$(debug)/$fileBase.o: \\
  $asmGNUFiles[$i] \\
  $cfg
	\@echo Compiling $asmGNUFiles[$i]...
	\$(asmGNUc) \$(asmGNUopts) -o $asmGNUFilesBase[$i].o $asmGNUFiles[$i]
	\@echo Collecting object files $asmGNUFilesBase[$i++].o...
	mv -f *.o \$(debug)
...
EOF
	...
	print MAKE <<EOF;
  $cfg
	\@echo Compiling $cFiles[$i]...
	\$(cc) \$(copts) $cFiles[$i]
	\@echo Collecting object files $cFilesBase[$i++].o...
	mv -f *.o \$(debug)
...
#******************************************************************************
#	This file has been generated by C Pattern Builder $toolver
#	2082 - Hau Sy Le - hau.le.eb\@renesas.com
#******************************************************************************
