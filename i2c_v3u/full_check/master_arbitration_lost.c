...
void i2c0InterruptHandler(void)
{
    ...
    else
    {
        ...
        {
            if (j < (DATA_PACKAGE_LENGTH - 2)) { receivedData[j++] = I2C_slaveReceiveMultipleByteNext(I2C0); }
            else if (j == (DATA_PACKAGE_LENGTH - 2)) { receivedData[j++] = I2C_slaveReceiveMultipleByteStop(I2C0); }
            else if (j == (DATA_PACKAGE_LENGTH - 1)) { receivedData[j++] = I2C_slaveReceiveMultipleByteFinish(I2C0); }
        }
...
void i2c1InterruptHandler(void)
{
    ...
    else if (status & I2C_INT_MDE) { I2C_masterSendMultipleByteNext(I2C1, sendData[i++]); }
